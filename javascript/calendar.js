...
const eventFor = document.getElementById('eventFor'); // Add this element in your modal form
...
async function getProfileAndFamily() {
  ...
  // Get family members for "event for" dropdown
  const { data: members } = await supabase.from('profiles').select('id,name').eq('family_id', familyId);
  familyMembers = members || [];
  // Populate eventFor dropdown
  if (eventFor) {
    eventFor.innerHTML = '';
...
function renderCalendar(month, year) {
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    dayEvents.forEach((e, idx) => {
      ...
      evDiv.innerHTML = `<span>${e.title || ''}</span>${e.location ? `<span style="font-size:0.9em;color:#fff8;"> @ ${e.location}</span>` : ''}${e.for ? `<span style="font-size:0.85em;color:#fff8;display:block;">For: ${getMemberName(e.for)}</span>` : ''}`;
...
function editEvent(idx) {
  ...
  if (eventFor) eventFor.value = e.for || '';
