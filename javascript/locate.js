...
// Show all family members' locations on the map, including you
async function showFamilyLocations() {
  if (!familyId || !map) return;
  // Clear all existing markers
  if (window._familyMarkers) {
    window._familyMarkers.forEach(m => map.removeLayer(m));
  }
  window._familyMarkers = [];
  const { data: members } = await supabase.from('profiles').select('full_name,lat,lng,id').eq('family_id', familyId);
  members.forEach(m => {
    if (typeof m.lat === 'number' && typeof m.lng === 'number') {
      const isYou = m.id === userProfile.id;
      const marker = L.marker([m.lat, m.lng], {
        icon: isYou
          ? L.icon({ iconUrl: 'https://cdn-icons-png.flaticon.com/512/149/149071.png', iconSize: [36, 36], iconAnchor: [18, 36] })
          : undefined
      }).addTo(map);
      marker.bindPopup(`<b>${m.full_name || "Unknown"}</b>${isYou ? " (You)" : ""}`);
      window._familyMarkers.push(marker);
    }
  });
}
...
async function setupMapAndLocations() {
  ...
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(async function(pos) {
      ...
      // Show all family locations (including you)
